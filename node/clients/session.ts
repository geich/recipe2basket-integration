...
export class Session extends JanusClient {
  ...
  public getSession = async (token: string, items: string[]) => {
    ...
    } = await this.http.postRaw<any>(routes.base, ({
      headers: {
  ...
  public createSession = async () => {
    const { data: { sessionToken, segmentToken }} = await this.http.postRaw<any>(routes.base, ({
      headers: {
        'Content-Type': 'application/json'
      },
      metric: 'session-create',
    }))

    return {
      sessionToken, segmentToken
    }
  }
